...
<div class="wrapper">
        ...
        <div class="col-md-3 table_info brr">
            <div class="col-md-12 baac1">
                <div class="col-md-4 nonestyle set">
                    <img class="game_ava set" src="/img/icon/11.png"/>
                </div>
                <div class="col-md-8 nonestyle">
                    <div class="">
                        <center><h3 class="ferma-gide-menu ttinfo"><?= Yii::t('app', 'Пирог с сыром') ?>
                            <a href="#" class="tooltip-info">
                                [?]
                                <span class="custom info"><em><?= Yii::t('app', 'Информация') ?></em>
                                    <?= Yii::t('app', 'Пекарня Пирог с сыром') ?><br>
                                    <?= Yii::t('app', 'Для приготовления 200 пирогов необходимо 70 теста и 200 сыра') ?><br>
                                    <?= Yii::t('app', 'Время переработки: 24 часа') ?><br>
                                    <?= Yii::t('app', 'Каждый запуск забирает 1300ед. энергии и дает 10 ед. опыта') ?><br>
                                    <?= Yii::t('app', 'Каждый сбор забирает 2600 ед. энергии и дает 10 ед. опыта') ?><br>
                                </span>
                            </a>
                        </h3></center>
                    </div>
            ...
            <div class="modal_content bgmodalses">
                <div class="modal-header">
                    <div class="col-md-1 col-mod-1">
                        <button type="button" class="close closes mod" data-dismiss="modal" aria-hidden="true"><?= Yii::t('app', 'Выйти') ?></button>
                    </div>
                </div>
    ...
    <div class="col-md-6 col-md-offset-3 nones-st">
        <?php foreach($factoryItems as $id => $factoryItem) : ?><?php
            $class = '';
            $click = '';
            $ready = false;
            $not_available = false;
            ...
            $tomato = \common\models\CheeseBakery::find()->where(['item_id' => $factoryItem['item_id']])->andWhere(['status_id'=>\common\models\CheeseBakery::STATUS_RUN])->all();
            $tomato = count($tomato);
            if($tomato>0) {
                $status = 'bmd-bg-warning';
            }

            $green = \common\models\CheeseBakery::find()->where(['item_id' => $factoryItem['item_id']])->andWhere(['status_id'=>\common\models\CheeseBakery::STATUS_READY_PRODUCT])->all();
            $green = count($green);
            if($green>0){
            ...
            {
                if($factoryItem['status_id'] == \common\models\CheeseBakery::STATUS_READY)
                {
                    $class = 'ready_cheese_bakery';
                    $ready = true;
                    $click = 'run_bakery';
                }
                elseif($factoryItem['status_id'] == \common\models\CheeseBakery::STATUS_RUN){
                    $class = 'ready_cheese_bakery';
                    $ready = true;
                    $click = 'check';
                    $productReady = 200;
                }
                elseif($factoryItem['status_id'] == \common\models\CheeseBakery::STATUS_READY_PRODUCT){
                    $class = 'ready_cheese_bakery';
                    $ready = true;
            ...
            $('body').on('click','a.build_bakery', function()
            {
                ...
                $.ajax({
                    ...
                    if (response.status) {
                        ...
                        $('.'+pole).addClass('ready_cheese_bakery');
                ...
                $.ajax({
                    ...
                    if (response.status) {
                        ...
                        if(response.label == true){
                            $('.'+pole).removeClass('ready_cheese_bakery');
