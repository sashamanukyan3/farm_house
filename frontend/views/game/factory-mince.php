...
<div class="wrapper">
        ...
        <div class="col-md-3 table_info brr">
            <div class="col-md-12 baac1">
                <div class="col-md-4 nonestyle set">
                    <img class="game_ava set" src="/img/icon/8.png"/>
                </div>
                <div class="col-md-8 nonestyle">
                    <div class="">
                        <center><h3 class="ferma-gide-menu ttinfo"><?= Yii::t('app', 'Фабрика фарша') ?>
                            <a href="#" class="tooltip-info">
                                [?]
                                <span class="custom info"><em><?= Yii::t('app', 'Информация') ?></em>
                                    <?= Yii::t('app', 'Фабрика фарша') ?><br>
                                    <?= Yii::t('app', 'Перерабатывает 300 мяса в 200 фарша! Время переработки: 24 часа') ?>.<br>
    ...
    <div class="col-md-6 col-md-offset-3 nones-st">
        <?php foreach($factoryItems as $id => $factoryItem) : ?>
            ...
            $tomato = \common\models\FactoryMince::find()->where(['item_id' => $factoryItem['item_id']])->andWhere(['status_id'=>\common\models\FactoryMince::STATUS_RUN])->all();
            $tomato = count($tomato);
            if($tomato>0) {
                $status = 'bmd-bg-warning';
            }

            $green = \common\models\FactoryMince::find()->where(['item_id' => $factoryItem['item_id']])->andWhere(['status_id'=>\common\models\FactoryMince::STATUS_READY_PRODUCT])->all();
            $green = count($green);
            if($green>0){
            ...
            {
                if($factoryItem['status_id'] == \common\models\FactoryMince::STATUS_READY)
                {
                    $class = 'ready_factory_mince';
                    $ready = true;
                    $click = 'run_factory';
                }
                elseif($factoryItem['status_id'] == \common\models\FactoryMince::STATUS_RUN){
                    $class = 'ready_factory_mince';
                    $ready = true;
                    $click = 'check';
                    $productReady = 200;
                }
                elseif($factoryItem['status_id'] == \common\models\FactoryMince::STATUS_READY_PRODUCT){
                    $class = 'ready_factory_mince';
                    $ready = true;
            ...
            $('body').on('click','a.build_factory', function()
            {
                ...
                $.ajax({
                    ...
                    if (response.status) {
                        ...
                        $('.'+pole).addClass('ready_factory_mince');
                ...
                $.ajax({
                    ...
                    if (response.status) {
                        if(response.label == true){
                            $('.'+pole).removeClass('ready_factory_mince');
