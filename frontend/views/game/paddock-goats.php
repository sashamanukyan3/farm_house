...
<div class="wrapper">
        ...
        <div class="col-md-3 table_info brr">
            <div class="col-md-12 baac1">
                <div class="col-md-4 nonestyle set">
                    <img class="game_ava set" src="/img/icon/2.png"/>
                </div>
                <div class="col-md-8 nonestyle">
                    <div class="">
                        <center><h3 class="ttinfo"><?= Yii::t('app', 'Загон коз') ?>
                            <a href="#" class="tooltip-info">
                                [?]
                                <span class="custom info"><em><?= Yii::t('app', 'Информация') ?></em>
                                    <?= Yii::t('app', 'В каждый загон можно посадить 9 коз') ?>.<br>
                                    <?= Yii::t('app', 'Животных нужно кормить 1 раз в день. На следующий день, Вы получите 2 единицы продукции, которую надо собрать. 1 коза дает 60 молока, после чего навсегда покидает игру') ?>.<br>
                                    ...
                                    <?= Yii::t('app', 'Каждый сбор продукта забирает 16 ед. энергии и дает 2 ед. опыта') ?>.<br>
                                    <?= Yii::t('app', 'Потребление энергии кнопкой автосбора') ?>:<br>
                                    <?= Yii::t('app', 'Каждое кормление забирает 8 ед. энергии и дает 2 ед. опыта') ?>.<br>
                                    <?= Yii::t('app', 'Каждый сбор продукта забирает 17 ед. энергии и дает 2 ед. опыта') ?>.<br>
                                </span>
                            </a>
                        </h3></center>
                    </div>
    ...
    <div class="modal fade bs-example-modal-lg" id="myZagon" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modalpes ples">
            <div class="modal_content bgmodalses">
                ...
                <div class="modal-body modal_padding_top goat_modal">
    ...
    <div class="col-md-6 col-md-offset-3 nones-st">
        <?php foreach($paddockItems as $id => $paddockItem) : ?>
            ...
            $goatItems = \common\models\GoatItems::find()->where(['paddock_id'=>$paddockItem['item_id']])->andWhere(['not', ['time_finish' => 0]])->andWhere(['<=', 'time_finish', strtotime('now')])->all();
            if($goatItems){
                foreach($goatItems as $goatItem) {
                    $goatItem->status_id = \common\models\GoatItems::STATUS_MILK;
                    $goatItem->save();
                }
            }
            ...
            $green = \common\models\GoatItems::find()->where(['paddock_id' => $paddockItem['item_id']])->andWhere(['status_id'=>\common\models\GoatItems::STATUS_MILK])->all();
            $green = count($green);
            if($green>0){
                $status = 'bmd-bg-success'; //green
            }

            $red = \common\models\GoatItems::find()->where(['paddock_id' => $paddockItem['item_id']])->andWhere(['status_id'=>\common\models\GoatItems::STATUS_HUNGRY])->all();
            $red = count($red);
            if($red>0){
            ...
            {
                if($paddockItem['status_id'] == \common\models\PaddockGoatItems::STATUS_READY)
                {
                    $class = 'ready_paddock_goat';
                    $ready = true;
                    $click = '';
                }
                else
            ...
            <div class="paddock-chicken pole-<?=$id + 1 .' '.$class ?>">
                ...
                <?php if($ready): ?><span class="badge <?=$status;?> status" id="status_<?=$paddockItem['item_id']; ?>">
                    <?php $count = count(\common\models\GoatItems::find()->where(['paddock_id'=>$paddockItem['item_id']])->andWhere(['not', ['status_id'=>0]])->all()); if($count!=0){echo $count;} else {echo 0;}?>
                    </span><?php endif; ?>
    ...
    <div class="modal" id="feedAll">
        <div class="modal-dialog-f bmd-modal-dialog">
            <div class="modal-content-f">
                ...
                <div class="modal-body">
                    <p class="response-answer3"><?= Yii::t('app', 'Вы уверены, что хотите покормить всех коз?') ?></p>
                </div>
                <div class="modal-ferma">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('body').on('click','a.build_paddock_goat', function()//покупка курятника
            {
                ...
                //alert(item_id);
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/build-paddock-goat/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'id': id,
                    ...
                    if (response.status) {
                        ...
                        $('.pole-'+item_id).addClass('ready_paddock_goat');
                        $('.pole-'+item_id+'>a').removeClass('build_paddock_goat');
            ...
            $('body').on('click','a.open-paddock', function()
            {
                ...
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/open-paddock-goat/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'paddockId': paddockId,
                    ...
                    if (response.status) {
                        ...
                        var status_class = '';
                        for(var i = 0; i < 9; i++) {
                            if(response.goatItems[i].status_id==2){
                                status_class = 'feed-goat';
                            } else if(response.goatItems[i].status_id==3){
                                status_class = 'goat-milk';
                            }else if(response.goatItems[i].status_id==1){
                                status_class = 'goat-default';
                            }else{
                                status_class = 'set-goat';
                            }
                            htmlElement = htmlElement + '<div data-position="'+response.goatItems[i].position+'" data-paddockid="'+paddockId+'" class="zagon-'+response.goatItems[i].position+ ' '+status_class+'">'+
                                    /*response.chickenItems[i].position +*/
                                '<div class="click-z">' +
                                '</div>'+
                                ((response.goatItems[i].status_id != 0) ? '<span class="badge bmd-bg-success status2">'+response.goatItems[i].count_milk/2+'</span>':'') +
                                '</div>';
                        }
                        $('.goat_modal').html(htmlElement);
                        //alert(response.msg);
                    } else {
                        $('#myModa').modal('show');
                        $('.response-answer').html(response.msg);
            ...
            $('body').on('click', 'div.set-goat', function(){
                ...
                //alert(position);
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/set-goat/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'paddockId': paddockId,
                    ...
                    if (response.status) {
                        $('.zagon-'+position).removeClass('set-goat');
                        $('.zagon-'+position).addClass('feed-goat');
                        $('#goat').html(response.goat);
            ...
            $('body').on('click', 'div.feed-goat', function(){
                ...
                //alert(position);
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/feed-goat/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'paddockId': paddockId,
                    ...
                    if (response.status) {
                        $('.zagon-'+position).removeClass('feed-goat');
                        $('.zagon-'+position).addClass('goat-default');
                        $('.user_energy').html(response.energy);
                        $('.user_experience').html(response.experience);
                        $('#feed_goats').html(response.feed);
            ...
            $('body').on('click', 'div.goat-milk', function(e){
                var position = $(this).data('position');
                var paddockId = $(this).data('paddockid');
                clicks++;  //count clicks
                if(clicks === 1) {
                    timer = setTimeout(function() {
                        ...
                        //alert(position);
                        $.ajax({
                            url: "<?= Url::toRoute('/paddock/get-goat-milk/') ?>",
                            type: "POST",
                            async: true,
                            data: {
                                'paddockId': paddockId,
            ...
            $('body').on('dblclick', 'div.goat-milk', function(e){
                e.preventDefault();  //cancel system double-click event
            });

            $('body').on('click', 'button#collect_all', function(){
                var modelName = 'GoatItems';
            ...
            $('body').on('click','.goat-default',function()
            {
                if(!IS_ENABLED_CHECK)
                {
                    return 1;
                ...
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/goat-check/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'bull_id': bull_id,
                    ...
                    if (response.status) {
                        element.removeClass('goat-default');
                        element.addClass('goat-milk');
                    } else {
