...
<div class="wrapper">
        ...
        <div class="col-md-3 table_info brr">
            <div class="col-md-12 baac1">
                <div class="col-md-4 nonestyle set">
                    <img class="game_ava set" src="/img/icon/12.png"/>
                </div>
                <div class="col-md-8 nonestyle">
                    <div class="">
                        <center><h3 class="ferma-gide-menu ttinfo"><?= Yii::t('app', 'Пирог с творогом') ?>
                            <a href="#" class="tooltip-info">
                                [?]
                                <span class="custom info"><em><?= Yii::t('app', 'Информация') ?></em>
                                    <?= Yii::t('app', 'Пекарня Пирог с творогом') ?><br>
                                    <?= Yii::t('app', 'Для приготовления 200 пирогов необходимо 70 теста и 200 творога') ?>.<br>
                                    <?= Yii::t('app', 'Время переработки: 24 часа') ?>.<br>
                                    <?= Yii::t('app', 'Каждый запуск забирает 1300ед. энергии и дает 10 ед. опыта') ?>.<br>
                                    <?= Yii::t('app', 'Каждый сбор забирает 3400 ед. энергии и дает 10 ед. опыта') ?>.<br>
                                </span>
                            </a>
                        </h3></center>
                    </div>
    ...
    <div class="col-md-6 col-md-offset-3 nones-st">
        <?php foreach($factoryItems as $id => $factoryItem) : ?>
            ...
            $tomato = \common\models\CurdBakery::find()->where(['item_id' => $factoryItem['item_id']])->andWhere(['status_id'=>\common\models\CurdBakery::STATUS_RUN])->all();
            $tomato = count($tomato);
            if($tomato>0) {
                $status = 'bmd-bg-warning';
            }

            $green = \common\models\CurdBakery::find()->where(['item_id' => $factoryItem['item_id']])->andWhere(['status_id'=>\common\models\CurdBakery::STATUS_READY_PRODUCT])->all();
            $green = count($green);
            if($green>0){
            ...
            {
                if($factoryItem['status_id'] == \common\models\CurdBakery::STATUS_READY)
                {
                    $class = 'ready_curd_bakery';
                    $ready = true;
                    $click = 'run_bakery';
                }
                elseif($factoryItem['status_id'] == \common\models\CurdBakery::STATUS_RUN){
                    $class = 'ready_curd_bakery';
                    $ready = true;
                    $click = 'check';
                    $productReady = 200;
                }
                elseif($factoryItem['status_id'] == \common\models\CurdBakery::STATUS_READY_PRODUCT){
                    $class = 'ready_curd_bakery';
                    $ready = true;
            ...
            $('body').on('click','a.build_bakery', function()
            {
                ...
                $.ajax({
                    ...
                    if (response.status) {
                        ...
                        $('.'+pole).addClass('ready_curd_bakery');
                ...
                $.ajax({
                    ...
                    if (response.status) {
                        ...
                        if(response.label == true){
                            $('.'+pole).removeClass('ready_curd_bakery');
