...
<div class="wrapper">
        ...
        <div class="col-md-3 table_info brr">
            <div class="col-md-12 baac1">
                <div class="col-md-4 nonestyle set">
                    <img class="game_ava set" src="/img/icon/3.png"/>
                </div>
                <div class="col-md-8 nonestyle">
                    <div class="">
                        <center><h3 class="ttinfo"><?= Yii::t('app', 'Загон коров') ?>
                            <a href="#" class="tooltip-info">
                                [?]
                                <span class="custom info"><em><?= Yii::t('app', 'Информация') ?></em>
                                    <?= Yii::t('app', 'В каждый загон можно посадить 9 коров') ?>.<br>
                                    <?= Yii::t('app', 'Животных нужно кормить 1 раз в день. На следующий день, Вы получите 2 единицы продукции, которую надо собрать. 1 корова дает 60 молока, после чего навсегда покидает игру') ?>.<br>
                                    <?= Yii::t('app', 'За одно кормление животное потребляет 2ед. корма') ?>.<br>
                                    <?= Yii::t('app', 'Каждое кормление забирает 10 ед. энергии и дает 2 ед. опыта') ?>.<br>
                                    <?= Yii::t('app', 'Каждый сбор продукта забирает 22 ед. энергии и дает 2 ед. опыта') ?>.<br>
                                    <?= Yii::t('app', 'Потребление энергии кнопкой автосбора') ?>:<br>
                                    <?= Yii::t('app', 'Каждое кормление забирает 11 ед. энергии и дает 2 ед. опыта') ?>.<br>
                                    <?= Yii::t('app', 'Каждый сбор продукта забирает 23 ед. энергии и дает 2 ед. опыта') ?>.<br>
                                </span>
                            </a>
                        </h3></center>
                    </div>
    ...
    <div class="modal fade bs-example-modal-lg" id="myZagon" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modalpes ples">
            <div class="modal_content bgmodalses">
                ...
                <div class="modal-body modal_padding_top cow_modal">
    ...
    <div class="col-md-6 col-md-offset-3 nones-st">
        <?php foreach($paddockItems as $id => $paddockItem) : ?>
            ...
            $cowItems = \common\models\CowItems::find()->where(['paddock_id'=>$paddockItem['item_id']])->andWhere(['not', ['time_finish' => 0]])->andWhere(['<=', 'time_finish', strtotime('now')])->all();
            if($cowItems){
                foreach($cowItems as $cowItem) {
                    $cowItem->status_id = \common\models\CowItems::STATUS_MILK;
                    $cowItem->save();
                }
            }
            ...
            $green = \common\models\CowItems::find()->where(['paddock_id' => $paddockItem['item_id']])->andWhere(['status_id'=>\common\models\CowItems::STATUS_MILK])->all();
            $green = count($green);
            if($green>0){
                $status = 'bmd-bg-success'; //green
            }

            $red = \common\models\CowItems::find()->where(['paddock_id' => $paddockItem['item_id']])->andWhere(['status_id'=>\common\models\CowItems::STATUS_HUNGRY])->all();
            $red = count($red);
            if($red>0){
            ...
            {
                if($paddockItem['status_id'] == \common\models\PaddockCowItems::STATUS_READY)
                {
                    $class = 'ready_paddock_cow';
                    $ready = true;
                    $click = '';
                }
                else
            ...
            <div class="paddock-chicken pole-<?=$id + 1 .' '.$class ?>">
                ...
                <?php if($ready): ?><span class="badge <?=$status;?> status" id="status_<?=$paddockItem['item_id']; ?>">
                    <?php $count = count(\common\models\CowItems::find()->where(['paddock_id'=>$paddockItem['item_id']])->andWhere(['not', ['status_id'=>0]])->all()); if($count!=0){echo $count;} else {echo 0;} ?>
                    </span><?php endif; ?>
                ...
                <?php if($ready): ?>
                    <ul class="dropdown-menu f-style">
                        <li><a href="#" class="price open-paddock-cow" data-toggle="modal" data-target="#myZagon" data-item_id="<?=$id+1?>" data-id="<?=$paddockItem['item_id']; ?>"><?= Yii::t('app', 'Открыть') ?></a></li>
    ...
    <div class="modal" id="collectAll">
        <div class="modal-dialog-f bmd-modal-dialog">
            <div class="modal-content-f">
                ...
                <div class="modal-body">
                    <p class="response-answer2"><?= Yii::t('app', 'Вы уверены, что хотите собрать все молоко?') ?></p>
                </div>
                <div class="modal-ferma">
    ...
    <div class="modal" id="feedAll">
        <div class="modal-dialog-f bmd-modal-dialog">
            <div class="modal-content-f">
                ...
                <div class="modal-body">
                    <p class="response-answer3"><?= Yii::t('app', 'Вы уверены, что хотите покормить всех коров?') ?></p>
                </div>
                <div class="modal-ferma">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('body').on('click','a.build_paddock_cow', function()//покупка курятника
            {
                ...
                //alert(item_id);
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/build-paddock-cow/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'id': id,
                    ...
                    if (response.status) {
                        ...
                        $('.pole-'+item_id).addClass('ready_paddock_cow');
                        $('.pole-'+item_id+'>a').removeClass('build_paddock_cow');
            ...
            $('body').on('click','a.open-paddock-cow', function()
            {
                $('.cow_modal').empty();
                var paddockId = $(this).data('id');
                var item_id = $(this).data('item_id');
                //alert(parseInt(timeNow));
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/open-paddock-cow/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'paddockId': paddockId,
                    ...
                    if (response.status) {
                        ...
                        var status_class = '';
                        for(var i = 0; i < 9; i++) {
                            if(response.cowItems[i].status_id==2){
                                status_class = 'feed-cow';
                            } else if(response.cowItems[i].status_id==3){
                                status_class = 'cow-milk';
                            }else if(response.cowItems[i].status_id==1){
                                status_class = 'cow-default';
                            }else{
                                status_class = 'set-cow';
                            }
                            htmlElement = htmlElement + '<div data-position="'+response.cowItems[i].position+'" data-paddockid="'+paddockId+'" class="zagon-'+response.cowItems[i].position+ ' '+status_class+'">'+
                                '<div class="click-z">' +
                                '</div>'+
                                ((response.cowItems[i].status_id != 0) ? '<span class="badge bmd-bg-success status2">'+response.cowItems[i].count_milk/2+'</span>':'') +
                                '</div>';
                        }
                        $('.cow_modal').html(htmlElement);
                        //alert(response.msg);
                    } else {
                        $('#myModa').modal('show');
                        $('.response-answer').html(response.msg);
            ...
            $('body').on('click', 'div.set-cow', function(){
                ...
                //alert(position);
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/set-cow/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'paddockId': paddockId,
                    ...
                    if (response.status) {
                        $('.zagon-'+position).removeClass('set-cow');
                        $('.zagon-'+position).addClass('feed-cow');
                        $('#cow').html(response.cow);
            ...
            $('body').on('click', 'div.feed-cow', function(){
                ...
                //alert(position);
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/feed-cow/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'paddockId': paddockId,
                    ...
                    if (response.status) {
                        $('.zagon-'+position).removeClass('feed-cow');
                        $('.zagon-'+position).addClass('cow-default');
                        $('.user_energy').html(response.energy);
                        $('.user_experience').html(response.experience);
                        $('#feed_cows').html(response.feed);
            ...
            $('body').on('click', 'div.cow-milk', function(){

            })

            var clicks = 0, timer = null;
            $('body').on('click', 'div.cow-milk', function(e){
                var position = $(this).data('position');
                var paddockId = $(this).data('paddockid');
                clicks++;  //count clicks
                if(clicks === 1) {
                    timer = setTimeout(function() {
                        ...
                        //alert(position);
                        $.ajax({
                            url: "<?= Url::toRoute('/paddock/get-cow-milk/') ?>",
                            type: "POST",
                            async: true,
                            data: {
                                'paddockId': paddockId,
                            ...
                            if (response.status) {
                                $('.zagon-'+position).removeClass('cow-milk');
                                $('.user_energy').html(response.energy);
                                $('.user_experience').html(response.experience);
                                if(response.status_id == 0){
                                    $('.zagon-'+position).addClass('set-cow');
                                    $('.zagon-'+position+'>span.status2').attr('style', 'display: none !important;');
                                }else{
                                    $('.zagon-'+position+'>span.status2').html(response.count_milk/2);
                                    $('.zagon-'+position).addClass('feed-cow');
                                }
                                $('#cow_milk_for_sell').html(response.milk);
                                if (response.showLevel == true) {
                                    response.msg += '<h4>';
                                    response.msg += '<?= mb_strtoupper(Yii::t('app', 'Вы достигли')) ?>';
                                    response.msg += ' ' + response.newLevel + ' ';
            ...
            $('body').on('dblclick', 'div.cow-milk', function(e){
                e.preventDefault();  //cancel system double-click event
            });

            $('body').on('click', 'button#collect_all', function(){
                var modelName = 'CowItems';
            ...
            $('body').on('click','.cow-default',function()
            {
                if(!IS_ENABLED_CHECK)
                {
                    return 1;
                ...
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/cow-check/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'bull_id': bull_id,
                    ...
                    if (response.status) {
                        element.removeClass('cow-default');
                        element.addClass('cow-milk');
                    } else {
