...
<div class="wrapper">
        ...
        <div class="col-md-3 table_info brr">
            <div class="col-md-12 baac1">
                <div class="col-md-4 nonestyle set">
                    <img class="game_ava set" src="/img/icon/4.png"/>
                </div>
                <div class="col-md-8 nonestyle">
                    <div class="">
                        <center><h3 class="ttinfo"><?= Yii::t('app', 'Загон бычков') ?>
                            <a href="#" class="tooltip-info">
                                [?]
                                <span class="custom info"><em><?= Yii::t('app', 'Информация') ?></em>
                                    <?= Yii::t('app', 'В каждый загон можно посадить 9 бычков') ?>.<br>
                                    <?= Yii::t('app', 'Животных нужно кормить 1 раз в день. На следующий день, Вы получите 2 единицы продукции, которую надо собрать') ?>.<br>
                                    <?= Yii::t('app', '1 бычок дает 60 мяса, после чего навсегда покидает игру') ?>.<br>
                                    <?= Yii::t('app', 'За одно кормление животное потребляет 2 ед. корма') ?>.<br>
                                    <?= Yii::t('app', 'Каждое кормление забирает 6 ед. энергии и дает 2 ед. опыта') ?>.<br>
                                    <?= Yii::t('app', 'Каждый сбор продукта забирает 10 ед. энергии и дает 2 ед. опыта') ?>.<br>
                                    <?= Yii::t('app', 'Потребление энергии кнопкой автосбора') ?>:<br>
                                    <?= Yii::t('app', 'Каждое кормление забирает 7 ед. энергии и дает 2 ед. опыта') ?>.<br>
                                    <?= Yii::t('app', 'Каждый сбор продукта забирает 11 ед. энергии и дает 2 ед. опыта') ?>.<br>
                                </span>
                            </a>
                        </h3></center>
                    </div>
    ...
    <div class="modal fade bs-example-modal-lg" id="myZagon" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modalpes ples">
            <div class="modal_content bgmodalses">
                ...
                <div class="modal-body modal_padding_top bull_modal">
                    <!--аХХХтоматические строим DOM-->
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-6 col-md-offset-3 nones-st">
        <?php foreach($paddockItems as $id => $paddockItem) : ?>
            ...
            $bullItems = \common\models\BullItems::find()->where(['paddock_id'=>$paddockItem['item_id']])->andWhere(['not', ['time_finish' => 0]])->andWhere(['<=', 'time_finish', strtotime('now')])->all();
            if($bullItems){
                foreach($bullItems as $bullItem) {
                    $bullItem->status_id = \common\models\BullItems::STATUS_MEAT;
                    $bullItem->save();
                }
            }
            ...
            $green = \common\models\BullItems::find()->where(['paddock_id' => $paddockItem['item_id']])->andWhere(['status_id'=>\common\models\BullItems::STATUS_MEAT])->all();
            $green = count($green);
            if($green>0){
                $status = 'bmd-bg-success'; //green
            }

            $red = \common\models\BullItems::find()->where(['paddock_id' => $paddockItem['item_id']])->andWhere(['status_id'=>\common\models\BullItems::STATUS_HUNGRY])->all();
            $red = count($red);
            if($red>0){
            ...
            {
                if($paddockItem['status_id'] == \common\models\PaddockBullItems::STATUS_READY)
                {
                    $class = 'ready_paddock_bull';
                    $ready = true;
                    $click = '';
                }
                else
            ...
            <div class="paddock-chicken pole-<?=$id + 1 .' '.$class ?>">
                ...
                <?php if($ready): ?><span class="badge <?=$status;?> status" id="status_<?=$paddockItem['item_id']; ?>">
                    <?php $count = count(\common\models\BullItems::find()->where(['paddock_id'=>$paddockItem['item_id']])->andWhere(['not', ['status_id'=>0]])->all()); if($count!=0){echo $count;} else {echo 0;}?>
                    </span><?php endif; ?>
                ...
                <?php if($ready): ?>
                    <ul class="dropdown-menu f-style">
                        <li><a href="#" class="price open-paddock-bull" data-toggle="modal" data-target="#myZagon" data-item_id="<?=$id+1?>" data-id="<?=$paddockItem['item_id']; ?>"><?= Yii::t('app', 'Открыть') ?></a></li>
    ...
    <div class="modal" id="collectAll">
        <div class="modal-dialog-f bmd-modal-dialog">
            <div class="modal-content-f">
                ...
                <div class="modal-body">
                    <p class="response-answer2"><?= Yii::t('app', 'Вы уверены, что хотите собрать все мясо?') ?></p>
                </div>
                <div class="modal-ferma">
    ...
    <div class="modal" id="feedAll">
        <div class="modal-dialog-f bmd-modal-dialog">
            <div class="modal-content-f">
                ...
                <div class="modal-body">
                    <p class="response-answer3"><?= Yii::t('app', 'Вы уверены, что хотите покормить всех бычков?') ?></p>
                </div>
                <div class="modal-ferma">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('body').on('click','a.build_paddock_bull', function()//постройка загона
            {
                ...
                //alert(item_id);
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/build-paddock-bull/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'id': id,
                    ...
                    if (response.status) {
                        ...
                        $('.pole-'+item_id).addClass('ready_paddock_bull');
                        $('.pole-'+item_id+'>a').removeClass('build_paddock_bull');
            ...
            $('body').on('click','a.open-paddock-bull', function()
            {
                $('.bull_modal').empty();
                ...
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/open-paddock-bull/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'paddockId': paddockId,
                    ...
                    if (response.status) {
                        ...
                        var status_class = '';
                        for(var i = 0; i < 9; i++) {
                            if(response.bullItems[i].status_id==2){
                                status_class = 'feed-bull';
                            } else if(response.bullItems[i].status_id==3){
                                status_class = 'bull-meat';
                            }else if(response.bullItems[i].status_id==1){
                                status_class = 'bull-default';
                            }else{
                                status_class = 'set-bull';
                            }
                            htmlElement = htmlElement + '<div data-position="'+response.bullItems[i].position+'" data-paddockid="'+paddockId+'" class="zagon-'+response.bullItems[i].position+ ' '+status_class+'">'+
                                    /*response.chickenItems[i].position +*/
                                '<div class="click-z">' +
                                '</div>'+
                                ((response.bullItems[i].status_id != 0) ? '<span class="badge bmd-bg-success status2">'+response.bullItems[i].count_meat/2+'</span>':'')+
                                '</div>';
                        }
                        $('.bull_modal').html(htmlElement);
                        //alert(response.msg);
                    } else {
                        $('#myModa').modal('show');
                        $('.response-answer').html(response.msg);
            ...
            $('body').on('click', 'div.set-bull', function(){
                ...
                //alert(position);
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/set-bull/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'paddockId': paddockId,
                    ...
                    if (response.status) {
                        $('.zagon-'+position).removeClass('set-bull');
                        $('.zagon-'+position).addClass('feed-bull');
                        $('#bull').html(response.bull);
            ...
            $('body').on('click', 'div.feed-bull', function(){
                ...
                //alert(position);
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/feed-bull/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'paddockId': paddockId,
                    ...
                    if (response.status) {
                        $('.zagon-'+position).removeClass('feed-bull');
                        $('.zagon-'+position).addClass('bull-default');
                        $('.user_energy').html(response.energy);
                        $('.user_experience').html(response.experience);
                        $('#feed_bulls').html(response.feed);
            ...
            $('body').on('click', 'div.bull-meat', function(e){
                var position = $(this).data('position');
                var paddockId = $(this).data('paddockid');
                clicks++;  //count clicks
                if(clicks === 1) {
                    timer = setTimeout(function() {
                        ...
                        $.ajax({
                            url: "<?= Url::toRoute('/paddock/get-meat/') ?>",
                            type: "POST",
                            async: true,
                            data: {
                                'paddockId': paddockId,
                            ...
                            if (response.status) {
                                $('.zagon-'+position).removeClass('bull-meat');
                                $('.user_energy').html(response.energy);
                                $('.user_experience').html(response.experience);
                                if(response.status_id == 0){//когда кол-во равно 60 статус обнуленным приходит сюда
                                    $('.zagon-'+position).addClass('set-bull');
                                    $('.zagon-'+position+'>span.status2').attr('style', 'display: none !important;');
                                }else{
                                    $('.zagon-'+position+'>span.status2').html(response.count_meat/2);
                                    $('.zagon-'+position).addClass('feed-bull');
                                }
                                $('#meat_for_sell').html(response.meat);
                                if (response.showLevel == true) {
                                    response.msg += '<h4>';
                                    response.msg += "<?= mb_strtoupper(Yii::t('app', 'Вы достигли')) ?>";
                                    response.msg += ' ' + response.newLevel + ' ';
                    ...
                    $.ajax({
                        ...
                        if (response.status) {
                            ...
                            if(response.status_id == 2){
                                ...
                                $('.zagon-'+position).removeClass('bull-meat');
                                $('.zagon-' + position).addClass('feed-bull');
                                $('#meat_for_sell').html(response.egg);
                            }
                        }
                        DISABLED_GET = true;
                        return 1;
            ...
            $('body').on('dblclick', 'div.bull-meat', function(e){
                e.preventDefault();  //cancel system double-click event
            });

            $('body').on('click', 'button#collect_all', function(){
                var modelName = 'BullItems';
            ...
            $('body').on('click','.bull-default',function()
            {
                if(!IS_ENABLED_CHECK)
                {
                    return 1;
                ...
                $.ajax({
                    url: "<?= Url::toRoute('/paddock/bull-check/') ?>",
                    type: "POST",
                    async: true,
                    data: {
                        'bull_id': bull_id,
                    ...
                    if (response.status) {
                        element.removeClass('bull-default');
                        element.addClass('bull-meat');
                    } else {
